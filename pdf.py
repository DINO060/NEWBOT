...
async def start_handler(client, message: Message):
    ...
    # (Cette section a été supprimée car les commandes multi-bannières ont été retirées)
...
async def button_callback(client, query: CallbackQuery):
    ...
    # Liste des callbacks qui font partie de flux multi-étapes
    ...
    # Vérifier si on est dans un flux multi-étapes
    ...
    # Si on est dans un flux multi-étapes OU en attente d'input, ne pas bloquer
    ...
    # IMPORTANT: Ajouter "both_full" dans les exclusions pour NE PAS définir processing=True
    if (
    ...
    if data == "download_link":
        ...
        await query.edit_message_text("🔗 Send me the link to download (PDF or Scribd URL).\nSend /cancel to abort.")
    ...
    if data == "settings":
        ...
        logger.info(f"🔍 Hashtag addition mode activated for user {user_id}")
        return
...
async def handle_all_text(client, message: Message):
            ...
            if is_valid_url(raw_text):
                ...
                if 'scribd.com' in url_l:
                    try:
                        ...
                        await client.send_message(message.chat.id, "❌ Failed to process Scribd URL.")
                    return
                if url_l.endswith('.pdf'):
                    try:
                        ...
                        await client.send_message(message.chat.id, "❌ Couldn't fetch the file from the URL.")
                    return
    except Exception:
        pass
    # === Download URL flow ===
    if session.get('awaiting_download_url'):
        ...
        if not (url.startswith('http://') or url.startswith('https://')):
            await client.send_message(message.chat.id, "❌ Please send a valid URL starting with http:// or https://")
            return
    ...
    if message.text:
        ...
        if txt.startswith("/download"):
            sess = ensure_session_dict(user_id)
            sess['awaiting_download_url'] = True
            await client.send_message(message.chat.id, "🔗 Send me the link to download (PDF or Scribd URL).\nSend /cancel to abort.")
            return
    ...
    if session.get("awaiting_new_filename"):
        ...
        if not new_name:
            await client.send_message(message.chat.id, "❌ Empty filename. Please try again.")
            return
        ...
        await client.send_message(message.chat.id, f"✅ File renamed to: {cleaned_name}")
        return
...
async def process_unlock(client, message, session, password):
    ...
    try:
        ...
        await client.send_message(message.chat.id, "✅ PDF unlocked successfully!")
        
    except pikepdf.PasswordError:
        await client.send_message(message.chat.id, "❌ Incorrect password. Please try again.")
        return
    except Exception as e:
        await client.send_message(message.chat.id, f"❌ Error unlocking PDF: {e}")
        return
    finally:
        session.pop('action', None)
...
async def process_pages(client, message, session, pages_text):
    ...
    try:
        ...
        if not pages_list:
            await client.send_message(message.chat.id, "❌ Invalid page specification. Please try again.")
            return
        ...
        await send_and_delete(client, message.chat.id, out_path, cleaned_name, delay_seconds=delay)
        await client.send_message(message.chat.id, f"✅ Pages {pages_list} removed successfully!")
        
    except Exception as e:
        await client.send_message(message.chat.id, f"❌ Error removing pages: {e}")
        return
    finally:
        session.pop('action', None)
...
async def on_photo_maybe_banner(client, message: Message):
    ...
    if not await is_user_in_channel(uid):
        await send_force_join_message(client, message)
        return
    # New: multi-banner add mode (supprimé car les commandes multi-bannières ont été retirées)
...
async def cmd_pdf_edit(client, message: Message):
    ...
    await client.send_message(message.chat.id, "🔄 Starting Full Process pipeline...")
...
async def process_clean_username(client, message, session):
    ...
    await send_and_delete(client, message.chat.id, in_path, final_name, delay_seconds=delay)
    await client.send_message(message.chat.id, "✅ Usernames cleaned from filename!")
...
async def process_rename_file(client, message, session):
    ...
    await client.send_message(
        message.chat.id,
        "📝 Send me the new name for your file (without .pdf extension):\n\n"
        "Examples:\n"
        "• My Document\n"
        "• Report 2024\n"
        "• Project Final"
